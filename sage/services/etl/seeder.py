...
from sage.models.cmo import CMORecordCreate
...
async def upsert_records(records: list[dict], program_code: str) -> tuple[int, int]:
    inserted, failed = 0, 0
    async with get_db() as conn:
        for record in records:
            try:
                ...
                await conn.execute(FAILED_SQL, program_code, str(record), str(e))
                failed += 1
...
async def seed_program(program_code: str) -> dict:
    ...
    if not program_name:
        return {"status": "unknown_program", "program": program_code}

    try:
        raw_records = await search_and_extract_cmo(program_name, program_code)

        if not raw_records:
            return {"status": "no_source_found", "program": program_code}

        inserted, failed = await upsert_records(raw_records, program_code)

        return {"status": "done", "program": program_code, "inserted": inserted, "failed": failed}

    except Exception as e:
        print(f"[seeder] Error seeding {program_code}: {e}")
        return {"status": "error", "program": program_code, "message": str(e)}
