...
async def extract_cmo_from_pdf(pdf_url: str, program_code: str) -> list[dict]:
    ...
    payload = {
        ...
        "settings": {
            ...
            "actions": [
                {"type": "download_file", "timeout": 30000},
                {
                    "type": "parse_json",
                    "data_schema": {
                        "name": "CHEDCurriculum",
    ...
    headers = {
        "X-API-Key": settings.gaffa_api_key or "",
        "Content-Type": "application/json",
    }

    async with httpx.AsyncClient() as client:
        response = await client.post(GAFFA_BASE_URL, json=payload, headers=headers, timeout=300.0)
    ...
    parse_action = next((a for a in data["data"]["actions"] if a["type"] == "parse_json"), None)
    ...
    for course in courses:
        tags = [
            t["tag"]
            for t in course.get("competency_tags", [])
...
async def search_and_extract_cmo(program_name: str, program_code: str) -> list[dict]:
    ...
    payload = {
        ...
        "settings": {
            ...
            "actions": [
                {"type": "wait", "selector": "body", "timeout": 5000, "continue_on_fail": True},
                {
                    "type": "parse_json",
                    "data_schema": {
                        "name": "SearchResults",
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(GAFFA_BASE_URL, json=payload, headers=headers, timeout=60.0)
        response.raise_for_status()
        data = response.json()
