...
async def create_semester_tree(program_code: str, year_level: int, semester: int, workspace_id: str, workspace_root_id: str = "") -> dict:
    ...
    if not workspace_root_id:
        ...
        if not pages:
            return {"status": "error", "message": "No accessible pages found! SAGE needs access to at least one Notion page to build under."}
    ...
    for course in commons:
        ...
        if competency_tags:
            topics_db_properties = {
                "Topic": {"title": {}},
                "Competency": {"select": {"options": [
                    {"name": str(tag), "color": "blue"} 
                    for tag in competency_tags
                ]}},
                "Status": {"select": {"options": [
            ...
            for topic_item in expanded_topics:
                ...
                entry = await notion_service.create_database_entry(
                    ...
                    properties={
                        "Topic": {
                            "title": [{
                                "text": {"content": topic_item["topic"]}
                            }]
                        },
                        "Competency": {
                            "select": {"name": topic_item["competency"]}
                        },
