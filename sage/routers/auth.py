...
async def notion_callback(code: str):
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.post(
            "https://api.notion.com/v1/oauth/token", headers=headers, json=payload
        )

    if resp.status_code != 200:
        raise HTTPException(
            status_code=resp.status_code, detail=f"Notion OAuth failed: {resp.text}"
        )
    ...
    query = """
        INSERT INTO user_tokens (workspace_id, bot_id, encrypted_token, root_page_id)
        VALUES ($1, $2, $3, NULL)
    ...
    response.set_cookie(key="workspace_id", value=workspace_id, httponly=True, samesite="lax")
    return response
...
async def get_current_user(request: Request) -> str:
    ...
    async with pool.acquire() as connection:
        exists = await connection.fetchval(
            "SELECT 1 FROM user_tokens WHERE workspace_id = $1", workspace_id
        )
        if not exists:
