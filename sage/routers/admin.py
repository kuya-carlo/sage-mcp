...
async def trigger_seed(request: SeedRequest, background_tasks: BackgroundTasks):
    ...
    return {"status": "seeding_started", "program": request.program_code}


@router.post("/upload", dependencies=[Depends(verify_admin)])
async def upload_syllabus(program_code: str, file: UploadFile | None = None):
...
async def delete_seed(program_code: str):
    ...
    return {"status": "deleted", "program": program_code}
